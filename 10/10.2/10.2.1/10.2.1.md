> [10. Objetos de Base de Datos](../../10.md) › [10.2. Vistas](../10.2.md) › [10.2.1. Módulo 1 / Integrante 1](10.2.1.md)

# 10.2.1. Módulo 1 / Integrante 1

# Vistas ​🖥️​
## Se da prioridad a consultas que se realizaran frecuentemente

### Vista pantalla de clientes: VISTA_CLIENTES_COMPLETA

```sql
CREATE OR REPLACE VIEW "FERRETERIA".VISTA_CLIENTES_COMPLETA AS
SELECT
    P.nombre_persona AS Nombre,
    D_LAT.distrito AS Distrito,
    T_LAT.valor_contacto AS Telefono,
    E_LAT.valor_contacto AS Correo,
    TO_CHAR(C.fecha_registro_cliente, 'YYYY-MM-DD') AS Fecha_Registro,
    P.COD_PERSONA 
FROM
    "FERRETERIA".CLIENTE C
JOIN
    "FERRETERIA".PERSONA P ON C.cod_persona = P.cod_persona

LEFT JOIN LATERAL (
    SELECT D.distrito
    FROM "FERRETERIA".DIRECCION_PERSONA DP
    JOIN "FERRETERIA".DIRECCION D ON DP.cod_direccion = D.cod_direccion
    WHERE DP.cod_persona = P.cod_persona AND DP.PRINCIPAL_DIRECCION = TRUE
) AS D_LAT ON TRUE

LEFT JOIN LATERAL (
    SELECT CO.valor_contacto
    FROM "FERRETERIA".CONTACTO_PERSONA COP
    JOIN "FERRETERIA".CONTACTO CO ON COP.cod_contacto = CO.cod_contacto
    JOIN "FERRETERIA".TIPO_CONTACTO TC ON CO.cod_tipo_contacto = TC.cod_tipo_contacto
    WHERE COP.cod_persona = P.cod_persona
      AND TC.valor_tipo_contacto = 'TELEFONO CELULAR'
      AND COP.PRINCIPAL_CONTACTO = (SELECT COD_TIPO_CONTACTO FROM "FERRETERIA".TIPO_CONTACTO WHERE valor_tipo_contacto = 'TELEFONO CELULAR')
) AS T_LAT ON TRUE

LEFT JOIN LATERAL (
    SELECT CO.valor_contacto
    FROM "FERRETERIA".CONTACTO_PERSONA COP
    JOIN "FERRETERIA".CONTACTO CO ON COP.cod_contacto = CO.cod_contacto
    JOIN "FERRETERIA".TIPO_CONTACTO TC ON CO.cod_tipo_contacto = TC.cod_tipo_contacto
    WHERE COP.cod_persona = P.cod_persona
      AND TC.valor_tipo_contacto = 'CORREO'
      AND COP.PRINCIPAL_CONTACTO = (SELECT COD_TIPO_CONTACTO FROM "FERRETERIA".TIPO_CONTACTO WHERE valor_tipo_contacto = 'CORREO')
) AS E_LAT ON TRUE;

SELECT * FROM "FERRETERIA".VISTA_CLIENTES_COMPLETA;
  ```
![alt text](images/image-6.png)
![alt text](images/image-9.png)

### Vista pantalla de maestros: VISTA_MAESTROS_COMPLETA

```sql
CREATE OR REPLACE VIEW "FERRETERIA".VISTA_MAESTROS_COMPLETA AS
SELECT
    P.nombre_persona AS Nombre,
    M.ruc AS RUC,
    D_LAT.distrito AS Distrito,
    T_LAT.valor_contacto AS Telefono,
    E_LAT.valor_contacto AS Correo,
    ES_LAT.valor_especialidad AS Especialidad,
    TO_CHAR(M.FECHA_REGISTRO_MAESTRO , 'YYYY-MM-DD') AS Fecha_Registro,
    P.COD_PERSONA 
FROM
    "FERRETERIA".MAESTRO M
JOIN
    "FERRETERIA".PERSONA P ON M.cod_persona = P.cod_persona


LEFT JOIN LATERAL (
    SELECT D.distrito
    FROM "FERRETERIA".DIRECCION_PERSONA DP
    JOIN "FERRETERIA".DIRECCION D ON DP.cod_direccion = D.cod_direccion
    WHERE DP.cod_persona = P.cod_persona AND DP.PRINCIPAL_DIRECCION = TRUE
) AS D_LAT ON TRUE

LEFT JOIN LATERAL (
    SELECT CO.valor_contacto
    FROM "FERRETERIA".CONTACTO_PERSONA COP
    JOIN "FERRETERIA".CONTACTO CO ON COP.cod_contacto = CO.cod_contacto
    JOIN "FERRETERIA".TIPO_CONTACTO TC ON CO.cod_tipo_contacto = TC.cod_tipo_contacto
    WHERE COP.cod_persona = P.cod_persona
      AND TC.valor_tipo_contacto = 'TELEFONO CELULAR'
      AND COP.PRINCIPAL_CONTACTO = (SELECT COD_TIPO_CONTACTO FROM "FERRETERIA".TIPO_CONTACTO WHERE valor_tipo_contacto = 'TELEFONO CELULAR')
) AS T_LAT ON TRUE

LEFT JOIN LATERAL (
    SELECT CO.valor_contacto
    FROM "FERRETERIA".CONTACTO_PERSONA COP
    JOIN "FERRETERIA".CONTACTO CO ON COP.cod_contacto = CO.cod_contacto
    JOIN "FERRETERIA".TIPO_CONTACTO TC ON CO.cod_tipo_contacto = TC.cod_tipo_contacto
    WHERE COP.cod_persona = P.cod_persona
      AND TC.valor_tipo_contacto = 'CORREO'
      AND COP.PRINCIPAL_CONTACTO = (SELECT COD_TIPO_CONTACTO FROM "FERRETERIA".TIPO_CONTACTO WHERE valor_tipo_contacto = 'CORREO')
) AS E_LAT ON TRUE

LEFT JOIN LATERAL (
    SELECT ES.valor_especialidad
    FROM "FERRETERIA".ESPECIALIDADES ES
    WHERE M.cod_especialidad = ES.cod_especialidad
) AS ES_LAT ON TRUE;

SELECT * FROM "FERRETERIA".VISTA_MAESTROS_COMPLETA;
  ```
![alt text](images/image-7.png)
![alt text](images/image-8.png)

### Vista de premios: VISTA_CATALOGO_PREMIOS

```sql
--VISTA_CATALOGO_PREMIOS
CREATE OR REPLACE VIEW VISTA_CATALOGO_PREMIOS AS
SELECT 
P.NOMBRE_PREMIO AS NOMBRE,
P.DESCP_PREMIO AS DESCRIPCION,
P.PUNTOS_PREMIO AS COSTO,
STRING_AGG(C.VALOR_CATEGORIA, ', ') AS CATEGORIAS,
P.DISPONIBILIDAD_PREMIO,
P.COD_PREMIO
FROM PREMIOS P 
LEFT JOIN CATEGORIAS_PREMIO CAP ON CAP.COD_PREMIO = P.COD_PREMIO 
LEFT JOIN CATEGORIA C ON CAP.COD_CATEGORIA = C.COD_CATEGORIA 
GROUP BY P.COD_PREMIO;

SELECT * FROM VISTA_CATALOGO_PREMIOS;
  ```
![alt text](images/image-11.png)
![alt text](images/image-10.png)


[🏠 Home](../../../README.md) | [Siguiente ➡️](../10.2.2/10.2.2.md)
