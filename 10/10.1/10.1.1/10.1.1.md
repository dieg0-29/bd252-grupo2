...
```sql
--CREAMOS INDICE
CREATE INDEX ON "FERRETERIA".PERSONA(NOMBRE_PERSONA);
...
SELECT * 
FROM "FERRETERIA".CONTACTO C 
WHERE C.VALOR_CONTACTO = '998 705 600';
```
![alt text](image.png)
```sql
--CREAMOS INDICE
CREATE INDEX ON "FERRETERIA".CONTACTO (VALOR_CONTACTO);
--CON INDICE
EXPLAIN ANALYZE
...
SELECT * 
FROM "FERRETERIA".DIRECCION D  
WHERE (D.CIUDAD, D.DISTRITO, D.VIA, D.NUMERO) = ('Huancayo', 'Concepción', 'Jr. Junín', '153');
```
![alt text](images/image-5.png)
```sql
--CREAMOS INDICE
CREATE INDEX ON "FERRETERIA".DIRECCION(CIUDAD, DISTRITO, VIA, NUMERO);
--CON INDICE
EXPLAIN ANALYZE
